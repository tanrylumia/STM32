...
//编码数据
//lpsSource 源数据
bool EncodeData(char *lpsSource,unsigned char len)
{
	...
	ncLength = ncSource > 0 ? ncSource : len;//计算要编码的数据的长度
//	if(ncLength>17) ncLength=17;
	DB_Print("str=%d\r\n",ncLength);
	//uprintf("ncLength = %d\n",ncLength);
	if (ncLength == 0)
		return FALSE; 
